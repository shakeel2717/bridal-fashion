    ...
    <div class="d-flex align-items-center justify-content-between mb-3">
        <div>
            ...
            <div class="page-subtitle">
                ...
                @if($sale->employee) · {{ $sale->employee->name }} @endif
            </div>
        </div>
    ...
    <div class="row g-3">
        <div class="col-8">
            ...
            <div class="table-card mb-3" style="padding:16px 20px;">
                ...
                <div class="row g-2" style="font-size:13px;">
                    ...
                    <div class="col-4">
                        ...
                        <div style="font-weight:600; font-family:monospace; font-size:12px;">{{ $sale->customer_cnic }}</div>
                    </div>
                    @endif
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="text-align:right; font-weight:700; color:var(--navy);">
                                Rs. {{ number_format(($item->sale_price * $item->qty) + $item->custom_option_price, 0) }}
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
            ...
            <div class="table-card" style="padding:14px 20px;">
                ...
                <div style="font-size:12px; white-space:pre-line; color:var(--text-primary);">{{ $sale->notes }}</div>
            </div>
            @endif
        ...
        <div class="col-4">
            ...
            <div class="table-card mb-3" style="padding:16px; border:1.5px solid #fed7d7;">
                <div style="font-size:11px; font-weight:700; color:#c53030; text-transform:uppercase; margin-bottom:12px;">
                    Record Refund
                </div>
                <div class="mb-2">
                    ...
                    <select wire:model.live="refundType" class="form-select form-select-sm">
                        ...
                        <option value="full">Full Refund (Rs. {{ number_format($sale->advance_paid, 0) }})</option>
                ...
                <div class="mb-3">
                    <label class="form-label">Note</label>
                    <textarea wire:model="refundNote" class="form-control form-control-sm"
                              rows="2" placeholder="Reason..."></textarea>
                </div>
                <button class="btn btn-sm btn-danger w-100"
                        wire:click="saveRefund">Save Refund</button>
            </div>
            @endif
            ...
            <div class="table-card" style="padding:14px 16px; border:1.5px solid #fed7d7; background:#fff5f5;">
                <div style="font-size:11px; font-weight:700; color:#c53030; margin-bottom:6px;">Refund Recorded</div>
                <div style="font-size:12px;">
                    Amount: <strong>Rs. {{ number_format($sale->refund_amount, 0) }}</strong><br>
                    @if($sale->refund_date) Date: {{ \Carbon\Carbon::parse($sale->refund_date)->format('d/m/Y') }}<br> @endif
                    @if($sale->refund_note) Note: {{ $sale->refund_note }} @endif
                </div>
            </div>
            @endif
