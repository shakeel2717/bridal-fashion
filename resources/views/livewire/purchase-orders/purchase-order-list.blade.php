<div>
    ...
    <div class="d-flex gap-2 mb-3 flex-wrap">
        <div
            style="background:#fff; border-radius:7px; padding:7px 14px; font-size:12px; border:1px solid var(--border);">
        ...
        @if ($totalBalance > 0)
            <div
                style="background:#fff5f5; border-radius:7px; padding:7px 14px; font-size:12px; border:1px solid #fed7d7;">
    ...
    <div class="table-card">
            ...
            <div class="d-flex gap-2 align-items-center">
                <div class="tab-pills" style="margin-bottom:0;">
                    <button class="tab-pill {{ $filterStatus === '' && $hasSearched ? 'active' : '' }}"
                        wire:click="$set('hasSearched', true); $set('filterStatus', '')">
                        All
                    </button>
                    ...
                    <button class="tab-pill {{ $filterStatus === 'ordered' ? 'active' : '' }}"
                        wire:click="$set('hasSearched', true); $set('filterStatus','ordered')">Ordered</button>
                    <button class="tab-pill {{ $filterStatus === 'partial' ? 'active' : '' }}"
                        wire:click="$set('hasSearched', true); $set('filterStatus','partial')">Partial</button>
                    <button class="tab-pill {{ $filterStatus === 'received' ? 'active' : '' }}"
                        wire:click="$set('hasSearched', true); $set('filterStatus','received')">Received</button>
                </div>
                <select wire:model.live="filterVendor" class="form-select form-select-sm" style="width:160px;">
        ...
        @if (!$hasSearched && !$search && !$filterStatus && !$filterVendor)
            <div style="text-align:center; padding:40px; color:var(--text-muted); font-size:13px;">
                ...
                Use filters or search above to view purchase orders.
            </div>
        @endif
