...
class CustomerVendorReport extends Component
{
    ...
    private function customerRows(): \Illuminate\Support\Collection
    {
    ...
    private function vendorRows(): \Illuminate\Support\Collection
    {
        return Vendor::when($this->search, fn($q) =>
                $q->where('name', 'like', "%{$this->search}%")
            )
            ->get()
            ->map(function ($vendor) {
