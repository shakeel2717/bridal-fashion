...
class RentalEdit extends Component
{
    ...
    public function mount(Rental $rental): void
    {
        ...
        if ($rental->stitching_date || $rental->stitching_instructions) {
            ...
            $this->stitchingDate = $rental->stitching_date ? Carbon::parse($rental->stitching_date)->format('Y-m-d') : '';
    ...
    public function nextStep(): void
    {
        ...
        if ($this->step === 3 && empty($this->items)) {
    ...
    public function prevStep(): void { $this->step = max(1, $this->step - 1); }
    ...
    public function setCustomerType(string $type): void
    {
        ...
        $this->customerName = $this->customerPhone1 = $this->customerPhone2 = '';
        $this->customerWhatsapp = $this->customerCnic = $this->deliveryAddress = '';
        $this->customerSearch = '';
    ...
    {
        if (strlen($this->customerSearch) < 2) { $this->foundCustomers = null; return; }
        $this->foundCustomers = Customer::where('is_walkin', false)
            ->where(fn($q) => $q->where('name', 'like', "%{$this->customerSearch}%")
                ->orWhere('phone1', 'like', "%{$this->customerSearch}%")
                ->orWhere('cnic', 'like', "%{$this->customerSearch}%"))
            ->limit(6)->get(['id', 'name', 'phone1', 'cnic'])->toArray();
    }
    ...
    public function selectCustomer(int $id): void
    {
        ...
        $this->customerPhone1 = $c->phone1;
        $this->customerPhone2 = $c->phone2 ?? '';
        $this->customerWhatsapp = $c->whatsapp ?? '';
        $this->customerCnic   = $c->cnic ?? '';
        $this->deliveryAddress = $c->address ?? '';
        $this->customerSearch = $c->name;
        $this->foundCustomers = null;
    }

    public function setGender(string $field, string $gender): void { $this->$field = $gender; }

    public function validateStep1(): void
    {
        $rules = ['customerName' => 'required|string|max:150', 'customerPhone1' => 'required|string|max:20'];
        if ($this->customerType === 'existing') $rules['customerCnic'] = 'required|string|max:20';
        $this->validate($rules);
    }
    ...
    public function searchProducts(): void
    {
        ...
        if (strlen($this->productSearch) < 1) { $this->searchResults = []; return; }

        $alreadyAdded = collect($this->items)->pluck('product_id')->toArray();

        $bookedProductIds = RentalItem::whereHas('rental', fn($q) =>
            $q->where('id', '!=', $this->rental->id)
    ...
    public function confirmAddItem(): void
    {
        ...
        $isBooked = RentalItem::whereHas('rental', fn($q) =>
            $q->where('id', '!=', $this->rental->id)
              ->whereNotIn('status', ['returned', 'cancelled', 'abandoned'])
              ->whereRaw('DATE(pickup_date) <= ?', [$this->returnDate])
              ->whereRaw('DATE(return_date) >= ?', [$this->pickupDate])
    ...
    public function removeItem(int $index): void { array_splice($this->items, $index, 1); $this->recalcTotal(); }

    public function addAddon(int $i): void { $this->items[$i]['addons'][] = ['label' => '', 'price' => '0']; }

    public function removeAddon(int $i, int $j): void { array_splice($this->items[$i]['addons'], $j, 1); $this->recalcTotal(); }

    public function addSecurityDeposit(): void { $this->securityDeposits[] = ['item_name' => '', 'amount' => '0', 'is_paid' => false]; }

    public function removeSecurityDeposit(int $i): void { array_splice($this->securityDeposits, $i, 1); }
    ...
    public function recalcTotal(): void
    {
        ...
        foreach ($this->items as $item) {
            ...
            foreach ($item['addons'] ?? [] as $addon) $subtotal += (float) ($addon['price'] ?? 0);
        }
        $discount = $this->discountType === 'percent'
            ? $subtotal * ((float) $this->discountValue / 100)
    ...
    public function save(): void
    {
        ...
        $walkinPhotoPath    = $this->rental->walkin_photo;
        $walkinCnicFrontPath = $this->rental->walkin_cnic_front;
        $walkinCnicBackPath  = $this->rental->walkin_cnic_back;

        if ($this->walkinPhoto)     $walkinPhotoPath     = $this->walkinPhoto->store('walkin/photos', 'public');
        if ($this->walkinCnicFront) $walkinCnicFrontPath = $this->walkinCnicFront->store('walkin/cnic', 'public');
        if ($this->walkinCnicBack)  $walkinCnicBackPath  = $this->walkinCnicBack->store('walkin/cnic', 'public');
        ...
        // Sync items — delete removed ones, update/create
        $keptIds = collect($this->items)->pluck('rental_item_id')->filter()->toArray();
        $this->rental->items()->whereNotIn('id', $keptIds)->delete();
        ...
        $this->rental->securityDeposits()->whereNotIn('id', $keptDepositIds)->delete();
        foreach ($this->securityDeposits as $deposit) {
            if (empty($deposit['item_name'])) continue;
            if (!empty($deposit['deposit_id'])) {
                $this->rental->securityDeposits()->find($deposit['deposit_id'])?->update([
                    'item_name' => $deposit['item_name'],
                    'amount'    => (float) $deposit['amount'],
                    'is_paid'   => (bool) $deposit['is_paid'],
                ]);
            } else {
                $this->rental->securityDeposits()->create([
                    'item_name'  => $deposit['item_name'],
                    'amount'     => (float) $deposit['amount'],
                    'is_paid'    => (bool) $deposit['is_paid'],
                    'is_refunded'=> false,
        ...
        // Stitching task sync
        $this->rental->tasks()->where('type', 'stitching')->delete();
    ...
    public function render()
    {
        ...
        $accounts  = Account::where('is_active', true)->orderByDesc('is_default')->orderBy('name')->get(['id', 'name', 'type']);
        return view('livewire.rentals.rental-edit', compact('employees', 'accounts'));
    }
}